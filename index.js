...
bot.hear(['Ask a question..'], (payload, chat) => {
  ...
  const askWhatSecondTime = (convo) => {
    convo.ask('Anything else you would like to ask?', (payload, convo) => {
      const text = payload.message.text;
      if(text.trim().slice(0,2) === 'no' || text.trim().slice(0,2) === 'No') convo.end();
      convo.say(`Thank you for the question. I'll forward it to my colleague Cameron, he'll answer as soon as possible.`);
      convo.end();
    });
  };
